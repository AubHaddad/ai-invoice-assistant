...
When the user asks for a spending report or chart (monthly, quarterly, or yearly; by category or vendor), call generateReport with period, groupBy, and optional currency. period is the current calendar month, quarter, or year. Amounts are already converted to a single currency — report only the rows and total this tool returns. Do not invent series or re-sum mixed currencies yourself.
